# Maria Alejandra Rujano alemaryrujano@gmail.com

# Answer 1. Merges the training and the test sets to create one data set. 
...
# Answer 4. Appropriately labels the data set with descriptive activity names. 

names(S) <- "subject" 
cleaned <- cbind(S, Y, X) 
write.table(cleaned, "merged_clean_data.txt") 

# Answer 5. Creates a 2nd, independent tidy data set with the average of each variable for each activity and each subject. 
...
for (s in 1:numSubjects) { 
       for (a in 1:numActivities) { 
             ...
             var <- cleaned[cleaned$subject==s & cleaned$activity==activities[a, 2], ] 
